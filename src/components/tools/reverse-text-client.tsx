...
import { reverseText } from "@/lib/tools/text-processing";
...
export default function ReverseTextClient() {
  ...
  return (
    <>
      <ToolInput label="Text to reverse" value={input} onChange={setInput} placeholder="Hello World" />
      ...
      <ActionButtons onClear={() => { setInput(""); setOutput(""); }} onProcess={() => setOutput(reverseText(input, mode))} processLabel="Reverse" disabled={!input.trim()} />
      <ToolOutput label="Reversed Text" value={output} downloadFilename="reversed.txt" />
    </>
  );
}
